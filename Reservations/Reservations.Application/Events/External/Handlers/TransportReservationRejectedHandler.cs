...
{
    public class TransportReservationRejectedHandler : IEventHandler<TransportReservationRejected>
    {
        private readonly IReservationRepository _repository;
        private readonly IMessageBroker _messageBroker;

        public TransportReservationRejectedHandler(IReservationRepository repository, IMessageBroker messageBroker)
        {
            _repository = repository;
            _messageBroker = messageBroker;
        }
        public async Task HandleAsync(TransportReservationRejected @event, CancellationToken cancellationToken = default)
        {
            var attemps = 3;
            while (attemps > 0)
            {
